...
Registry.prototype = {
            ...
            .then(function (client) {
                ...
                function _registry() {
                    ...
                    client.exists(registryPathFolder, function (error, stat) {
                        if (error) {
                            console.error('RegistryPathFolder error.' + registryPathFolder);
                            process.exit(1);
                            return;
                        }
                        ...
                        if (stat) {
                            ...
                            client.create(registryPath, null, Zookeeper.CreateMode.EPHEMERAL, function (err) {
                                if (err) {
                                    ...
                                    console.log('Registry : 注册成功 [' + invokerDesc + ']');
                                }
                            ...
                            client.create(registryPathFolder, null, Zookeeper.CreateMode.PERSISTENT, function (err) {
                                if (err) {
                                    console.error('RegistryPathFolder create error.' + registryPathFolder);
                                } else {
                                    //创建完成则重新调用该方法
                                    _registry();
                                }
        ...
        function getChildrenList(client) {
            client.getChildren(path, function () {
                ...
                if (err) {
                    ...
                    console.log('Registry : 获取权重失败 [' + serviceName + '] [Configurator protocol empty.Resume all providers.]');
        ...
        children.forEach(function (child) {
            ...
            if ((mDisabled !== null || mEnable !== null) && oHost !== null) {
                if (invoker.isMatch(mGroup && mGroup[1], mVersion && mVersion[1])) {
                    invoker.refreshProviders(oHost[1],
    ...
    destroy: function () {
        ...
        console.log('zookeeper disposed');
    }
};
